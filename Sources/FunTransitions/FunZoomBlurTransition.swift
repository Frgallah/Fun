...
class FunZoomBlurTransition: FunCoreImageTransition {
    
    var firstFilter: CIFilter?
    var secondFilter: CIFilter?
    let minValue: Float = 0.0
    let maxValue: Float = 40.0
    var range: Float = 0
    ...
    override func setupTranisition(firstImage: CIImage?, secondImageView: UIView) {
        ...
        guard let filter1 = CIFilter.init(name: "CIZoomBlur") else { return  }
        filter1.setDefaults()
        filter1.setValue(NSNumber.init(value: minValue), forKey: "inputAmount")
        rect1 = image1.extent
        ...
        guard let filter2 = CIFilter.init(name: "CIZoomBlur") else { return  }
        filter2.setDefaults()
        filter2.setValue(NSNumber.init(value: maxValue), forKey: "inputAmount")
    ...
    override func updateFirstFilter(fraction: CGFloat) {
        firstFilter?.setValue(filterValue(fracton: Float(fraction)), forKey: "inputAmount")
    ...
    override func updateSecondFilter(fraction: CGFloat) {
        secondFilter?.setValue(filterValue(fracton: Float(fraction)), forKey: "inputAmount")
    ...
    override func drawFirstImage() {
        firstFilter?.setValue(NSNumber.init(value: 0), forKey: "inputAmount")
        outputImage = firstFilter?.outputImage
        drawingView?.setNeedsDisplay()
    }
    
    override func drawSecondImage() {
        secondFilter?.setValue(NSNumber.init(value: 0), forKey: "inputAmount")
        outputImage = secondFilter?.outputImage
        drawingView?.setNeedsDisplay()
    }
