//
//  FunKaleidoscopeTransition.swift
//  Pods
//
...
class FunKaleidoscopeTransition: FunCoreImageTransition {
    
    var firstFilter: CIFilter?
    var secondFilter: CIFilter?
    let minValue: Float = 2.0
    let maxValue: Float = 12.0
    var range: Float = 0
    ...
    override func setupTranisition(firstImage: CIImage?, secondImageView: UIView) {
        ...
        guard let filter1 = CIFilter.init(name: "CIKaleidoscope") else { return  }
        filter1.setDefaults()
        filter1.setValue(NSNumber.init(value: minValue), forKey: "inputCount")
        rect1 = image1.extent
        ...
        guard let filter2 = CIFilter.init(name: "CIKaleidoscope") else { return  }
        filter2.setDefaults()
        filter2.setValue(NSNumber.init(value: maxValue), forKey: "inputCount")
    ...
    override func updateFirstFilter(fraction: CGFloat) {
        firstFilter?.setValue(filterValue(fracton: Float(fraction)), forKey: "inputCount")
    ...
    override func updateSecondFilter(fraction: CGFloat) {
        secondFilter?.setValue(filterValue(fracton: Float(fraction)), forKey: "inputCount")
