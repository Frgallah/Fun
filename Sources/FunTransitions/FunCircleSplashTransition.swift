//
//  FunCircleSplashTransition.swift
//  Pods
//
...
class FunCircleSplashTransition: FunCoreImageTransition {
    ...
    override func setupTranisition(firstImage: CIImage?, secondImageView: UIView) {
        ...
        guard let filter1 = CIFilter.init(name: "CICircleSplashDistortion") else { return  }
        filter1.setDefaults()
        rect1 = image1.extent
        ...
        guard let filter2 = CIFilter.init(name: "CICircleSplashDistortion") else { return  }
        filter2.setDefaults()
        filter2.setValue(NSNumber.init(value: minValue), forKey: kCIInputRadiusKey)
        ...
        maxValue = Float(CGFloat.maximum(CGFloat.maximum(rect1.width, rect2.width), CGFloat.maximum(rect1.height, rect2.height)))
        range = maxValue - minValue
        filter1.setValue(NSNumber.init(value: maxValue), forKey: kCIInputRadiusKey)
        if isInteractive {
    ...
    func filterValue(fracton: Float) -> NSNumber {
        return NSNumber.init(value: minValue + range * (1.0 - (fracton * 2.0)))
    }
