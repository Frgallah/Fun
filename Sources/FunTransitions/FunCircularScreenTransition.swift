//
//  FunCircularScreenTransition.swift
//  Pods
//
...
class FunCircularScreenTransition: FunCoreImageTransition {
    ...
    override func setupTranisition(firstImage: CIImage?, secondImageView: UIView) {
        ...
        guard let filter1 = CIFilter.init(name: "CICircularScreen") else { return  }
        filter1.setDefaults()
        filter1.setValue(NSNumber.init(value: minValue), forKey: kCIInputWidthKey)
        rect1 = image1.extent
        ...
        guard let filter2 = CIFilter.init(name: "CICircularScreen") else { return  }
        filter2.setDefaults()
        filter2.setValue(NSNumber.init(value: maxValue), forKey: kCIInputWidthKey)
    ...
    override func updateFirstFilter(fraction: CGFloat) {
        firstFilter?.setValue(filterValue(fracton: Float(fraction)), forKey: kCIInputWidthKey)
    ...
    override func updateSecondFilter(fraction: CGFloat) {
        secondFilter?.setValue(filterValue(fracton: Float(fraction)), forKey: kCIInputWidthKey)
